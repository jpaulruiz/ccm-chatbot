...
function getSnippet(framework: Framework, src: string): string {
  switch (framework) {
    ...
    case 'Next.js':
      return `import Script from 'next/script';\n\n// Add inside <body> in your root layout (app/layout.tsx)\n<Script\n  src="${src}"\n  strategy="afterInteractive"\n/>`;
    case 'React':
...
export default function BotDetail({ bot: initial }: { bot: Bot }) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-12 max-w-6xl">
        ...
        {tab === 'Knowledge' && (
          <div className="space-y-4">
            ...
            <textarea
              ...
              rows={14}
              placeholder="Paste your info here — fees, opening hours, FAQs, policies, staff details…"
      ...
      <div className="hidden xl:block">
        <div className="sticky top-8 self-start bg-[#0a0f1b] border border-[#1f2940] rounded-2xl p-5">
          <p className="text-xs text-[#9aa7bd] mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-green-400 inline-block animate-pulse" />
            Live Preview — click the bubble
          </p>

        {/* Browser mockup */}
        <div className="rounded-xl overflow-hidden border border-[#1f2940] shadow-xl">
          {/* Browser chrome */}
          <div className="px-3 py-2.5 bg-[#0c1220] border-b border-[#1f2940] flex items-center gap-2">
            <div className="flex gap-1.5">
              <span className="w-3 h-3 rounded-full bg-red-500/70" />
              <span className="w-3 h-3 rounded-full bg-yellow-500/70" />
              <span className="w-3 h-3 rounded-full bg-green-500/70" />
            </div>
            <div className="flex-1 bg-[#0a0f1b] rounded-md px-3 py-1 text-[11px] text-[#54607a] flex items-center gap-1.5">
              <span>🌐</span> yourwebsite.com
            </div>
          </div>

          {/* Fake website content */}
          <div className="relative bg-[#070b12] h-[480px] overflow-hidden">
            {/* Placeholder content skeleton */}
            <div className="p-5 space-y-3 opacity-30">
              <div className="h-6 w-2/3 bg-[#1f2940] rounded" />
              <div className="h-3 w-full bg-[#1f2940] rounded" />
              <div className="h-3 w-5/6 bg-[#1f2940] rounded" />
              <div className="h-3 w-4/5 bg-[#1f2940] rounded" />
              <div className="mt-4 h-24 w-full bg-[#1f2940] rounded-lg" />
              <div className="grid grid-cols-3 gap-2 mt-2">
                <div className="h-16 bg-[#1f2940] rounded-lg" />
                <div className="h-16 bg-[#1f2940] rounded-lg" />
                <div className="h-16 bg-[#1f2940] rounded-lg" />
              </div>
              <div className="h-3 w-full bg-[#1f2940] rounded" />
              <div className="h-3 w-3/4 bg-[#1f2940] rounded" />
            </div>

            {/* Chat panel */}
            {previewOpen && (
              <div className="absolute bottom-16 right-3 w-[280px] h-[360px] rounded-2xl overflow-hidden shadow-2xl border border-[#1f2940]">
                <iframe
                  src={`/embed/${bot.id}`}
            ...
            <button
              ...
              className="absolute bottom-4 right-4 w-12 h-12 rounded-full shadow-lg flex items-center justify-center text-white text-xl transition-transform hover:scale-110 active:scale-95"
              style={{ background: grad }}
              title="Toggle chat"
            >
              {previewOpen ? '✕' : '💬'}
          ...
          <p className="text-[11px] text-[#54607a] mt-4 text-center">
